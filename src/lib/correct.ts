...
## 第二步：逐句批注（sentenceCorrections）

对原文中的**每一句话**给出点评（按要求拆分句子）：
- original: 原句
- revised: 改进后（如有语法或用词错误则修改；如无问题则写原句）
- comment: 点评（中文，指出优点或具体问题。如果该句没有错误，请给予正面评价，如"此句表达准确，语法正确，用词恰当"）

## 第三步：语法问题（grammarIssues，2-5个）

⚠️ 语法点去重规则：本质相同的语法点必须合并为一个。例如"时态不一致"、"时态一致性"、"时态错误"统一为"时态问题"；"主谓不一致"、"主谓一致错误"统一为"主谓一致问题"；"冠词缺失"、"冠词多余"、"冠词误用"统一为"冠词使用问题"。合并后在explanation中覆盖所有子问题。宁可少而精准，不多而浮夸。

找出典型语法问题：
如果文章没有明显的语法问题，grammarIssues可以为空数组。不要编造不存在的语法错误。
- point: 语法点名称（中文）
- level: 难度级别（基础/进阶/高级）
- function: 该语法点的正确功能
- structure: 正确结构公式
