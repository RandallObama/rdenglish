...
4. **对翻译结果进行逐句点评**：将中文原文按句拆分，对应英文翻译逐句评
...
## 逐句翻译点评要求
- 将中文原文按句号/分号拆分，对每句给出对应的英文翻译
- 对每句翻译的质量进行评价（准确度、地道程度、是否忠实原意）
- 如有更好的翻译方式，给出改进建议
...
严格返回以下 JSON（不要 markdown 代码块）:
{
  ...
  "sentenceReviews": [
    {
      "sourceSentence": "中文原句",
      "translatedSentence": "对应英文翻译",
      "quality": "质量评价（中文）",
      "suggestions": "改进建议，如无需改进则为空字符串"
    }
  ]
}

## 重要原则
- **语法点去重规则**：本质相同的语法点必须合并为一个。例如"时态不一致"、"时态一致性"、"时态错误"统一为"时态问题"；"主谓不一致"、"主谓一致错误"统一为"主谓一致问题"；"冠词缺失"、"冠词多余"、"冠词误用"统一为"冠词使用问题"。合并后选一个能概括所有子问题的名称，在explanation中覆盖全部子问题
- 语法点挑选2-4个最关键的即可，宁缺毋滥，每个深度分析
- 词汇挑选5-7个最有价值的，优先选考试高频词，覆盖不同难度层次
- 例句要自然地道，不要生造
- 讲解语言用中文，便于理解
- **宁缺毋滥**：如果翻译后的英文没有明显的语法问题，grammarNotes可以为空数组，不要强行编造不存在的语法点
- 只返回 JSON，不要其他文字`;
}
