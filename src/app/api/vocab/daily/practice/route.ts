/**
 * POST /api/vocab/daily/practice — 提交造句，AI 评价
 */
...
import { evaluateSentence } from "@/lib/vocab-daily";
...
export async function POST(request: Request) {
  ...
  if (!sessionId || wordIndex === undefined || !sentence?.trim()) {
    return NextResponse.json(
      { error: "缺少必要参数（sessionId, wordIndex, sentence）" },
      { status: 400 }
    );
  }
  ...
  if (!vocabSession.usageConsumed) {
    ...
    if (!usage.allowed) {
      return NextResponse.json(
        { error: `今日免费次数已用完（${usage.limit} 次）。明天再来吧！` },
        { status: 429 }
      );
    }
  ...
  if (!currentWord) {
    return NextResponse.json({ error: "找不到目标单词" }, { status: 400 });
  }
  ...
  try {
    ...
    console.error("[vocab-daily/practice] Error:", err);
    return NextResponse.json(
      { error: err?.message || "评价失败，请稍后重试" },
      { status: 500 }
    );
  }
}
