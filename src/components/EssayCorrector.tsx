...
import { Send, GraduationCap, Loader2, Wand2, Sparkles, X } from "lucide-react";
...
export function EssayCorrector({ onResult, onError }: EssayCorrectorProps) {
  ...
  const handleCowrite = async () => {
    if (!essay.trim() || cowriteLoading) return;
    ...
    try {
      const res = await fetch("/api/cowrite", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: essay.trim(), lang: "en", stream: false }),
        signal: abortController.signal,
      });
  ...
  const handlePickSuggestion = (suggestion: string) => {
    const separator = essay.trim() && !essay.endsWith(" ") ? " " : "";
    setEssay((prev) => prev + separator + suggestion);
    setShowSuggestions(false);
    toast.success("已追加续写");
  };
